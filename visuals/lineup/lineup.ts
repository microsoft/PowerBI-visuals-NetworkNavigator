...
LineUp.create = function (storage, $container, options) {
    ...
    var r = new (<any>LineUpClass)(storage, $container, options);
    r.startVis();
    return r;
};
...
LayoutStringColumn.prototype = $.extend({}, LayoutSingleColumn.prototype, {
    ...
    var r : any = this.getValue(row);
    if (typeof r === 'boolean') {
        return r && (<any>r).trim().length > 0;
...
LineUp.prototype.stackedColumnOptionsGui = function (selectedColumn) {
    ...
    popup.select('.ok').on('click', function() {
        var newValue = $('#popupInputText').val();
...
// } else {
//     this.LineUp = LineUpLoader(jQuery, d3, <any>_);
// }
export default LineUpLoader(jQuery, d3, <any>_);